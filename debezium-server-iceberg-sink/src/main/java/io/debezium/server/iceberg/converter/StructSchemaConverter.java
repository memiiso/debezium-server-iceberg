...
/**
 * Converts Kafka Connect Schemas (org.apache.kafka.connect.data.Schema) used in Debezium Struct
 * events into an Iceberg Schema.
 *
...
public class StructSchemaConverter implements SchemaConverter {
   ...
   * Handles nested structures (Struct, Map, Array) recursively. Mirrors
   * JsonSchemaConverter.debeziumFieldToIcebergField(JsonNode, String, IcebergSchemaInfo, JsonNode).
   *
   ...
   */
  private void debeziumFieldToIcebergField(
      Field connectField, IcebergSchemaInfo schemaData, Field connectKeyField) {
    ...
    switch (fieldType) {
      case STRUCT:
        if (config.iceberg().nestedAsVariant()) {
          ...
          final Types.NestedField variantField =
              Types.NestedField.optional(
                  variantFieldId, fieldName, Types.VariantType.get(), connectSchema.doc());
        ...
        for (Field subField : connectSchema.fields()) {
          debeziumFieldToIcebergField(
              subField, subSchemaData, equivalentKeyField(connectKeyField, subField.name()));
        }
        // Create it as struct, nested type
        final Types.StructType structType = Types.StructType.of(subSchemaData.fields());
        final Types.NestedField structField =
            Types.NestedField.of(
                rootStructId, isOptional, fieldName, structType, connectSchema.doc());
        ...
        if (isPkField) {
          ...
          LOGGER.warn(
              "Struct field '{}' is marked as PK. Complex types as identifiers might cause issues.",
              fieldName);
      ...
      case MAP:
        ...
        Type keyType =
            keySchemaData.fields().get(0).type(); // Assuming the recursive call adds one field
        if (keyType.isNestedType()) {
          throw new DebeziumException(
              "Invalid key type for map: "
                  + keyType.typeId()
                  + ". Map keys cannot be List, Map, or Struct.");
        }
        ...
        Type valueType =
            valSchemaData.fields().get(0).type(); // Assuming the recursive call adds one field

        final Types.MapType mapField =
            Types.MapType.ofOptional(keyFieldId, valFieldId, keyType, valueType);
        schemaData
            .fields()
            .add(
                Types.NestedField.of(
                    rootMapId, isOptional, fieldName, mapField, connectSchema.doc()));
        break; // Added break

      case ARRAY:
        if (isPkField) {
          throw new DebeziumException(
              "Array field '" + fieldName + "' cannot be part of the primary key.");
        }
        ...
        Type elementType =
            arraySchemaData.fields().get(0).type(); // Assuming the recursive call adds one field
        ...
        schemaData
            ...
            .add(
                Types.NestedField.of(
                    rootArrayId, isOptional, fieldName, listField, connectSchema.doc()));
        break; // Added break
      ...
      default:
        ...
        final Types.NestedField field =
            Types.NestedField.of(
                primitiveFieldId, isOptional, fieldName, primitiveType, connectSchema.doc());
  ...
  private Schema keyFieldSchema() {
    if (!config.debezium().isEventFlatteningEnabled() && keySchema != null) {
      ...
      // However, determining PKs reliably this way is problematic. JsonSchemaConverter throws an
      // exception later.
      // We'll get the key names but the check below will likely fail if PKs are found.
      LOGGER.warn(
          "Processing unflattened event schema. Identifier field determination might be unreliable.");
  ...
  private IcebergSchemaInfo icebergSchemaFields(
      Schema valueSchema, Schema keySchema, IcebergSchemaInfo schemaData) {
  ...
  /**
   * Converts the Kafka Connect value and key schemas to an Iceberg Schema. Mirrors
   * JsonSchemaConverter.icebergSchema().
   *
  ...
  public org.apache.iceberg.Schema icebergSchema(boolean withIdentifierFields) {
    if (this.valueSchema == null || this.valueSchema.type() != Schema.Type.STRUCT) {
      throw new DebeziumException(
          "Failed to get schema from Debezium event, value schema is null or not a Struct: "
              + valueSchema);
    }
  ...
  public SortOrder sortOrder(org.apache.iceberg.Schema schema) {
    ...
    for (Field field : getSchemaFields(keyFieldSchema())) {
      ...
      if (schema.findField(field.name()) == null) {
        LOGGER.debug(
            "Skipping sort order field '{}' as it does not exist in the Iceberg schema",
            field.name());
        continue;
      }
  ...
  /**
   * Maps Kafka Connect primitive and logical types to Iceberg primitive types. Mirrors
   * JsonSchemaConverter.icebergPrimitiveField(String, String, String, JsonNode).
   *
  ...
  private Type.PrimitiveType icebergPrimitiveField(String fieldName, Schema connectSchema) {
    ...
    switch (fieldType) {
      ...
      case INT32: // int 4 bytes
        ...
        // if (org.apache.kafka.connect.data.Time.LOGICAL_NAME.equals(logicalTypeName) || ...) {
      ...
      case INT64: // long 8 bytes
        ...
        // NOTE: Time type is disabled for the moment, it's not supported by spark
        // if (io.debezium.time.MicroTime.SCHEMA_NAME.equals(logicalTypeName) || ...) { return
        // Types.TimeType.get(); }
      ...
      case STRING:
        ...
        // NOTE: Time type is disabled for the moment, it's not supported by spark
        // if (io.debezium.time.ZonedTime.SCHEMA_NAME.equals(logicalTypeName) || ...) { return
        // Types.TimeType.get(); }
      ...
      case BYTES:
        ...
        if (org.apache.kafka.connect.data.Decimal.LOGICAL_NAME.equals(logicalTypeName)) {
          ...
          if (params != null
              && params.containsKey(org.apache.kafka.connect.data.Decimal.SCALE_FIELD)) {
            int scale =
                Integer.parseInt(params.get(org.apache.kafka.connect.data.Decimal.SCALE_FIELD));
            // Attempt to get precision, default if missing
            int precision =
                Integer.parseInt(params.getOrDefault("connect.decimal.precision", "38"));
            // Basic validation
            if (precision <= 0) {
              LOGGER.warn(
            ...
            LOGGER.warn(
                "Decimal logical type for field '{}' is missing scale parameter. Defaulting to Decimal(38, 10).",
                fieldName);
      ...
      default:
        ...
        LOGGER.warn(
            "Unsupported Kafka Connect schema type '{}' for field '{}'. Defaulting to String.",
            fieldType,
            fieldName);
        return Types.StringType.get();
    }
