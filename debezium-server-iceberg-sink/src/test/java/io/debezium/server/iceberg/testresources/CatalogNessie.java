...
public class CatalogNessie implements QuarkusTestResourceLifecycleManager {
  ...
  private GenericContainer<?> nessieContainer =
      new GenericContainer<>(DockerImageName.parse(NESSIE_IMAGE))
  ...
  public Map<String, String> start() {
    ...
    double durationSeconds =
        TimeUnit.NANOSECONDS.toMillis(endTime - startTime)
            / 1000.0; // Convert nanoseconds to seconds
    System.out.println("Nessie started: " + getNessieUri() + " duration: " + durationSeconds);

    Map<String, String> config = new ConcurrentHashMap<>();

    config.put("debezium.sink.iceberg.catalog-impl", "org.apache.iceberg.nessie.NessieCatalog");
  ...
  public String getNessieUri() {
    if (nessieContainer != null && nessieContainer.isRunning()) {
      return "http://"
          + nessieContainer.getHost()
          + ":"
          + nessieContainer.getMappedPort(NESSIE_PORT);
    }
    return null;
  }
}
