...
| `debezium.sink.iceberg.force-pk-optional`                    | `false`                                                       | When set to `true`, primary key fields will be created as optional in the Iceberg schema. This setting is only effective when `preserve-required-property` is `false`. By default, primary key fields are required.                                                                                               |
